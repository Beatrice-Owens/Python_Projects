...
def load_gui(self): #self = parent window
    ...
    #Define the listbox with a scrollbox and grid them
    #.bind creates a listener for when someone clicks/selects an item from the list
    #then calls onSelect() from phonebook_func
    self.scrollbar1 = Scrollbar(self.master,orient=VERTICAL)
    self.lstList1 = Listbox(self.master,exportselection=0,yscrollcommand=self.scrollbar1.set)
    self.lstList1.bind('<<ListboxSelect>>',lambda event: phonebook_func.onSelect(self,event)) #event = bound event
