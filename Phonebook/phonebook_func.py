...
def center_window(self, w, h): #pass in tkinter frame (master) reference and the w and h
    ...
    screen_width = self.master.winfo_screenwidth()  #tkinter method
    screen_height = self.master.winfo_screenheight()
    #calculate x and y coords to paint the app centered on user's screen
...
#asking the user if they really want to close
def ask_quit(self):
    #tkinter's messagebox with method "askokcancel(boxtitle, boxbody)
    if messagebox.askokcancel("Exit program", "Okay to exit application?"): 
        #closes the app
        self.master.destroy() #tkinter destroy method
        os._exit(0)  #prevents memory leaks by releasing widget memory space


#===============================================
def create_db(self):
    #class object called conn
...
def onSelect(self,event):
    ...
    select = varList.curselection()[0]  #catches the index clicked on but doesn't tell the text name
    value = varList.get(select)   #get text of the previous index number
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cursor = conn.cursor()
        cursor.execute("""SELECT col_fname,col_name,col_phone,col_email FROM tbl_phonebook WHERE col_fullname = (?)""", [value])
        varBody = cursor.fetchall()  #accessing and storing information out of the dB
        #returns a tuple that can be sliced into 4 parts using data[] during iteration
        for data in varBody:
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) >0) and (len(var_email) > 0): #user must provide both names
        conn - sqlite3.connect("phonebook.db")
        with conn:
            cursor = conn.cursor()
            # check dB for existence of the fullname, if so, alert user and disregard request
            cursor.execute("""SELECT COUNT(col_fullname) FROM tbl_phonebook WHERE col_fullname = '{}'""".format(var_fullname)) #,(var_fullname))
            count = cursor.fetchone()[0]
            chkName = count
            if chkName == 0:  #if this is 0 then there is no existence of the fullname and we add new data
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) #listbox's selected values
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor()
        #check count to ensure that this is not the last record in
        #the dB... cannot delete last record or an error occurs
        cur.execute("""SELECT COUNT(*) FROM tbl_phonebook""")
        count = cur.fetchone()[0]
        if count > 1:
...
def onDeleted(self):
    ...
    self.txt_phone.delete(0,END)
    self.txt_email.delete(0,END)
    onRefresh(self) #update the listbox for the changes
    try:
...
def onUpdate(self):
    try: #make sure the selection exists
        ...
        messagebox.showinfo("Missing selection", "No naem was selected from the list box. \nCancelling the update request.")
        return
    ...
    var_phone = self.txt_phone.get().strip() #normalize the data to maintain integrity
    var_email = self.txt_email.get().strip()
